      ...
      (event: StreamEvent) => {
        ...
        switch (event.type) {
          ...
          case 'llm_response_stop': {
            ...
            console.log(
              `   📊 Stats: ${responseLength} chars, ${duration}ms, ${toolCalls} tool calls`
            );
            break;
          }
          ...
          case 'tool_output':
            console.log(
              `[${eventCount}] 📊 Tool Result: ${JSON.stringify(event.data.output).substring(0, 100)}...`
            );
            break;
...
async function multipleStreams() {
  ...
  const messages = ['What is the weather in Paris?', 'Create a plan for learning TypeScript'];
