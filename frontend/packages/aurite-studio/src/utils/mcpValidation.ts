import { MCPServerConfig, MCPServerStdioConfig, MCPServerHttpConfig, MCPServerLocalConfig, MCPServerFormFields } from '../types';
...
export function validateMCPServerConfig(config: MCPServerConfig): string[] {
  ...
  if (config.transport_type === "stdio") {
    if (!config.server_path) errors.push("server_path is required for stdio transport");
    if (config.http_endpoint) errors.push("http_endpoint not allowed for stdio transport");
    if (config.command) errors.push("command not allowed for stdio transport");
  } else if (config.transport_type === "http_stream") {
    if (!config.http_endpoint) errors.push("http_endpoint is required for http_stream transport");
    if (config.server_path) errors.push("server_path not allowed for http_stream transport");
    if (config.command) errors.push("command not allowed for http_stream transport");
  } else if (config.transport_type === "local") {
    if (!config.command) errors.push("command is required for local transport");
    if (config.server_path) errors.push("server_path not allowed for local transport");
    if (config.http_endpoint) errors.push("http_endpoint not allowed for local transport");
  }
