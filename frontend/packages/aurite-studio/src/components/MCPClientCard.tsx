...
import { useClientConfigComplete, useRegisterMCPServer, useUnregisterMCPServer } from '@/hooks/useClients';
...
export const MCPClientCard: React.FC<MCPClientCardProps> = ({
  client,
  index,
  onConfigure,
}) => {
  ...
  const isDisconnecting = unregisterMutation.isPending && unregisterMutation.variables === clientName;
    ...
    >
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <div className={`w-2 h-2 rounded-full transition-colors duration-200 ${getStatusDotColor()}`} />
        </div>
        <p className="text-sm text-muted-foreground">
          {client.configFile ? 'Configured' : 'Configuration pending'}
        </p>
        <p className="text-xs text-muted-foreground capitalize">
          Status: {isConnecting ? 'connecting...' : isDisconnecting ? 'disconnecting...' : client.status}
        </p>
      </div>
