    ...
    if (request.stream) {
      ...
      await executeAgentStream.executeStream(
        ...
        event => {
          ...
          // Handle tool call events - only if agent has tools configured
          if (event.type === 'tool_call') {
            console.log('🔧 Tool call detected:', event);

            // Check if this agent actually has tools configured
            const agentHasTools = agentConfig?.mcp_servers && agentConfig.mcp_servers.length > 0;
            ...
            if (agentHasTools) {
              ...
              console.log('🚫 Ignoring tool call for agent without configured tools');
            }
            return;
          }

          // Handle tool output events - only if agent has tools configured
          if (event.type === 'tool_output') {
            ...
            if (agentHasTools) {
              ...
              console.log('🚫 Ignoring tool output for agent without configured tools');
            }
            return;
          }
      ...
      try {
        ...
        // Extract tool calls from conversation history - only if agent has tools configured
        const agentHasTools = agentConfig?.mcp_servers && agentConfig.mcp_servers.length > 0;
        const extractedToolCalls =
          agentHasTools && (result as any).history
        ...
        if (!agentHasTools && (result as any).history) {
          console.log('🚫 Ignoring tool calls for agent without configured tools');
        }
