...
async def test_llm_security_tester():
    ...
    test_llm_config = {"provider": "openai", "model": "gpt-3.5-turbo", "temperature": 0.7, "max_tokens": 1000}
    ...
    for _test_id, test in available_tests.items():
...
async def test_security_engine():
    ...
    llm_config = {"provider": "openai", "model": "gpt-3.5-turbo", "temperature": 0.7, "max_tokens": 1000}
    ...
    full_config = {
        "llm": {
            "primary_llm": {"provider": "openai", "model": "gpt-4", "temperature": 0.5, "max_tokens": 2000},
            "secondary_llm": {
                "provider": "anthropic",
                "model": "claude-3",
                "temperature": 0.3,
    ...
    results = await engine.assess_full_configuration(
        configuration=full_config, assessment_options={"tests": ["llm_config_audit", "token_limit_check"]}
    )

    print("   Full Configuration Assessment:")
    print(f"   - Components assessed: {len(results)}")

    for component_key, result in results.items():
        print(f"   - {component_key}: Score {result.overall_score:.2f}, Threats {len(result.threats)}")
...
async def test_configuration_management():
    ...
    config_dict = {
        ...
        "components": {"llm": {"enabled": True, "test_timeout_seconds": 45}},
        "api": {"enabled": True, "port": 8002},
    }
