...
from aurite.security.core.security_engine import SecurityEngine, SecurityAssessmentResult
...
class SecurityAssessmentRunner:
    ...
    async def assess_all_configurations(
        ...
        if parallel:
            ...
            for config_name, config in configurations.items():
                config_name, result = await self.assess_configuration(
                    config_name, config, assessment_options
                )
...
class ConfigurationSecurityAnalyzer:
    ...
    def identify_security_patterns(
        ...
        patterns['model_security_ranking'] = sorted(
            model_scores, 
        ...
        for result in results.values():
            for threat in result.threats:
                threat_type = threat.get('type', 'unknown') if isinstance(threat, dict) else getattr(threat, 'threat_type', 'unknown')
        ...
        patterns['common_threats'] = dict(sorted(
            threat_counts.items(), 
            key=lambda x: x[1], 
            reverse=True
        ))
...
class SecurityReportGenerator:
        ...
        for config_name, result in sorted_results:
            ...
            if total_threats > 0:
                ...
                report_lines.extend([
                    ...
                    f"🚨 Critical Threats: {len(critical_threats)}" if critical_threats else "🚨 Critical Threats: 0",
                    "",
    ...
    def _generate_config_specific_recommendations(self, config: Dict[str, Any], result: SecurityAssessmentResult) -> List[str]:
        ...
        if system_prompt:
            ...
            recommendations.append("No default system prompt defined. Consider adding a security-focused system prompt to establish AI behavior boundaries.")
        
        # Temperature-based recommendations
        temperature = config.get('temperature')
        if temperature is not None:
            if temperature > 1.0:
                recommendations.append(f"High temperature ({temperature}) may increase unpredictability. Consider reducing to 0.7-1.0 for better security.")
            elif temperature > 0.8:
                recommendations.append(f"Temperature ({temperature}) is moderately high. Monitor outputs for consistency.")
        
        # Token limit recommendations
        max_tokens = config.get('max_tokens')
        if max_tokens is not None:
            if max_tokens > 4000:
                recommendations.append(f"High token limit ({max_tokens}) may allow verbose responses. Consider limiting to 2000-4000 for better control.")
            elif max_tokens < 100:
                recommendations.append(f"Very low token limit ({max_tokens}) may truncate important responses. Consider increasing to 100-500.")
        ...
        if api_base:
            ...
            if 'localhost' in api_base or '127.0.0.1' in api_base:
                recommendations.append("Using localhost API endpoint may indicate development configuration. Ensure this is intentional for production.")
        ...
        if provider == 'openai':
            ...
            if temperature and temperature > 0.7:
                recommendations.append("Anthropic models work well with lower temperatures (0.3-0.7) for more consistent outputs.")
    ...
    def _analyze_system_prompt_security(self, system_prompt: str) -> List[str]:
        ...
        if len(system_prompt) < 20:
            recommendations.append(f"System prompt is very short ({len(system_prompt)} chars). Consider adding more specific security guidelines and behavior constraints.")
        elif len(system_prompt) > 500:
            recommendations.append(f"System prompt is very long ({len(system_prompt)} chars). Consider condensing to essential security and behavior guidelines.")
        
        # Check for security-related keywords
        security_keywords = [
            'security', 'safe', 'harmful', 'dangerous', 'illegal', 'ethical', 
            'appropriate', 'guidelines', 'rules', 'policy', 'compliance'
        ]
        ...
        if security_mentions == 0:
            recommendations.append("System prompt lacks explicit security guidelines. Consider adding instructions about safe, ethical, and appropriate responses.")
        elif security_mentions < 2:
            recommendations.append("System prompt has minimal security guidance. Consider strengthening with more explicit safety and ethical guidelines.")
        
        # Check for overly permissive language
        permissive_phrases = [
            'anything', 'everything', 'any request', 'all questions', 'no limits',
            'no restrictions', 'unlimited', 'unrestricted'
        ]
        
        for phrase in permissive_phrases:
            if phrase in prompt_lower:
                recommendations.append(f"System prompt contains potentially permissive language ('{phrase}'). Consider adding appropriate boundaries and limitations.")
        ...
        if not has_role:
            recommendations.append("System prompt should clearly define the AI's role and purpose for better user expectations and security.")
        ...
        if not has_instruction_guidance:
            recommendations.append("Consider adding guidance about following appropriate instructions while maintaining safety boundaries.")
        ...
        if 'ignore' in prompt_lower and ('instruction' in prompt_lower or 'prompt' in prompt_lower):
            recommendations.append("System prompt contains language about ignoring instructions, which could be exploited for prompt injection attacks.")
        ...
        if not has_data_guidance:
            recommendations.append("Consider adding guidance about handling personal, private, or sensitive information appropriately.")
        
        return recommendations
    
    def _serialize_for_json(self, obj: Any, visited: Optional[set] = None, max_depth: int = 10, current_depth: int = 0) -> Any:
            ...
            if isinstance(obj, dict):
                ...
                for attr_name in dir(obj):
                    ...
                    try:
                        ...
                        serialized[attr_name] = self._serialize_for_json(attr_value, visited, max_depth, current_depth + 1)
