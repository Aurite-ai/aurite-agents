...
async def search_workflow_history(workflow_name: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            response = await client.get(
                f"{API_BASE_URL}/execution/history", headers=HEADERS, params={"limit": 50}, timeout=10.0
            )
            response.raise_for_status()
            result = response.json()
            print(f"   Total sessions: {result['total']}")
...
async def check_workflow_config(workflow_name: str):
    ...
    if workflow_config:
        print("✅ Found workflow configuration:")
...
async def main():
    ...
    if result.get("session_id"):
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.get(
                    f"{API_BASE_URL}/execution/history/{result['session_id']}", headers=HEADERS, timeout=10.0
                )
                response.raise_for_status()
