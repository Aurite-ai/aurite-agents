...
def test_wheel_installation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = subprocess.run([str(pip_exe), "install", str(wheel_path)], capture_output=True, text=True)
...
"""
        ...
        result = subprocess.run(
            [str(python_exe), "-c", test_script], capture_output=True, text=True, env={**os.environ, "API_KEY": "dummy"}
        )
