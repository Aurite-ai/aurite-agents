...
def get_static_assets_info() -> dict:
    ...
    if not static_dir:
        return {"available": False, "path": None, "files": 0, "size_mb": 0}
    ...
    try:
        ...
        return {"available": False, "path": str(static_dir), "files": 0, "size_mb": 0}
...
def setup_studio_routes_with_static(app: FastAPI) -> bool:
    ...
    if success:
        ...
        async def redirect_to_studio():
            ...
            return HTMLResponse(content='<script>window.location.href="/studio";</script>', status_code=200)
