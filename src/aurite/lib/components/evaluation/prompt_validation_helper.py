...
async def run_iterations(
    ...
    test_input = (
        [testing_config.user_input] if type(testing_config.user_input) is not list else testing_config.user_input
    )

    tasks = [
        _run_single_iteration(executor, testing_config, t_in, prompts, i, override_system_prompt)
        for i in range(num_iterations)
        for t_in in test_input
    ]

    results_tuple = await asyncio.gather(*tasks)
    results, agent_responses = zip(*results_tuple, strict=False)
...
async def improve_prompt(executor: "AuriteEngine", model: str, results: list, current_prompt: str) -> str:
...
async def _get_agent_result(
    ...
    if testing_config.test_type == "agent":
        ...
        output = full_output.primary_text
    else:
...
def _extract_tool_calls(agent_response: dict) -> list[dict]:
    ...
    conversation_history = agent_response.get("conversation_history", agent_response.get("conversation", []))
                ...
                if tool_call.get("type") == "function":
                    ...
                    if function.get("name"):
                        try:
                            ...
                            arguments = {}
                            logger.warning(f"Failed to decode tool call arguments: {function.get('arguments')}")
                        tool_calls.append({"name": function["name"], "input": arguments})
