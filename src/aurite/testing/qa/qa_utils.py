...
def generate_cache_key(
    ...
    logger.debug(f"Generated cache key: {cache_key} for input: {case_input[:50]}...")
    return cache_key
...
async def get_cached_case_result(
    ...
    Args:
        ...
        session_manager: SessionManager instance for cache access
    ...
    try:
        ...
        cached_data = session_manager.get_qa_case_result(cache_key)
        if not cached_data:
            return None
        ...
        logger.warning(f"Error retrieving cached result for key {cache_key}: {e}")
...
async def store_cached_case_result(
    ...
    try:
        ...
        success = session_manager.save_qa_case_result(cache_key, cache_data)
        if success:
            logger.debug(f"Cached result for key: {cache_key}")
        return success

    except Exception as e:
        logger.warning(f"Error storing cached result for key {cache_key}: {e}")
        return False
...
def generate_evaluation_cache_key(
    ...
    for case in test_cases:
        case_data = {
            ...
            "expectations": sorted(case.expectations) if hasattr(case, "expectations") and case.expectations else None,
        }
    ...
    logger.debug(f"Generated evaluation cache key: {cache_key} for config: {evaluation_config_id}")
    return cache_key
...
async def get_cached_evaluation_result(
    ...
    try:
        # Try to get cached result
        cached_data = session_manager.get_qa_evaluation_result(cache_key)
        if not cached_data:
            return None
        ...
        cached_at = datetime.fromisoformat(cached_data.get("cached_at", ""))
        if datetime.utcnow() - cached_at > timedelta(seconds=cache_ttl):
            logger.debug(f"Evaluation cache expired for key: {cache_key}")
            return None
        ...
        result_data = cached_data.get("result")
        if result_data:
            logger.debug(f"Evaluation cache hit for key: {cache_key}")
            return QAEvaluationResult(**result_data)

    except Exception as e:
        logger.warning(f"Error retrieving cached evaluation result for key {cache_key}: {e}")
...
async def store_cached_evaluation_result(
    ...
    try:
        ...
        success = session_manager.save_qa_evaluation_result(cache_key, cache_data)
        if success:
            logger.debug(f"Cached evaluation result for key: {cache_key}")
        return success

    except Exception as e:
        logger.warning(f"Error storing cached evaluation result for key {cache_key}: {e}")
        return False
