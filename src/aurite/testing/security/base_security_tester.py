...
class BaseSecurityTester(ABC):
    ...
    async def assess_security(
        ...
        # Get enabled tests
        enabled_tests = [
            test for test in self._security_tests.values() if test.enabled and self._should_run_test(test, options)
        ]
        ...
        assessment = self._calculate_assessment_results(
            component_id=component_id, test_results=test_results, start_time=start_time, options=options
        )
    ...
    def _calculate_assessment_results(
        ...
        component_recommendations = self._get_component_specific_recommendations(test_results, all_threats, options)
    ...
    def _get_component_specific_recommendations(
        ...
        if failed_tests:
            recommendations.append(f"Review and address {len(failed_tests)} failed security tests")
        ...
        if high_threats:
            recommendations.append(f"Prioritize resolution of {len(high_threats)} high-severity threats")
    ...
    def get_enabled_tests(self) -> Dict[str, SecurityTest]:
        ...
        return {test_id: test for test_id, test in self._security_tests.items() if test.enabled}
