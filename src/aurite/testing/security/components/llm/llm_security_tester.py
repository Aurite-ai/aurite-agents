...
class LLMSecurityTester(BaseSecurityTester):
    ...
    async def _test_prompt_injection_basic(
        ...
        if detection_failures > 0:
            recommendations.append(
                f"Improve prompt injection detection - missed {detection_failures} out of {len(test_prompts)} test cases"
            )
    ...
    async def _test_toxicity_detection(
        ...
        return SecurityTestResult(
            ...
            score=average_score,
            threats_detected=threats_detected,
            recommendations=["Toxicity detection is functional"]
            if average_score >= 0.6
            else ["Improve toxicity detection sensitivity"],
            execution_time_seconds=0.0,
