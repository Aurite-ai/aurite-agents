"""
LLM Client for Portkey Gateway.
"""
...
class GatewayClient(BaseLLM):
    """LLM Client implementation for Portkey Gateway."""
    ...
    def __init__(
        ...
        if not os.environ.get(provider_to_key[provider]):
            logger.error(
                f"{provider_to_key[provider]} environment variable not found. "
                    ...
                    for block in content:
                        if block.get("type") == "text":
                            ...
                            tool_calls_for_api.append({
                                ...
                                "function": {
                                    "name": block.get("name"),
                                    "arguments": json.dumps(block.get("input", {})), # OpenAI expects arguments as JSON string
                                }
                            })
    ...
    async def create_message(
        ...
        if message_from_openai.content: # Text content
            output_content_blocks.append(
                AgentOutputContentBlock(type="text", text=str(message_from_openai.content))
            )
        ...
        if message_from_openai.tool_calls:
            for tool_call in message_from_openai.tool_calls:
                ...
                try:
                    ...
                    logger.error(f"Failed to parse JSON arguments for tool {tool_call_data.function.name}: {tool_call_data.function.arguments}")
                    arguments = {"error": "failed to parse arguments", "raw_arguments": tool_call_data.function.arguments}
