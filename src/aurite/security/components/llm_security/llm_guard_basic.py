...
class LLMGuardBasic:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Basic configuration
        self.input_scanners = config.get("input_scanners", [
            "PromptInjection", "Toxicity", "Secrets"
        ])
        self.output_scanners = config.get("output_scanners", [
            "Toxicity", "Sensitive", "MaliciousURLs"
        ])
    ...
    async def scan_input(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            results["threats"].append({
                "type": "scan_error",
                "severity": "high",
                "description": f"Input scanning failed: {str(e)}"
            })
    ...
    async def scan_output(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            results["threats"].append({
                "type": "scan_error",
                "severity": "high",
                "description": f"Output scanning failed: {str(e)}"
            })
    ...
    async def scan_conversation(self, messages: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        try:
            ...
            results["statistics"] = {
                ...
                "average_score": sum(r["score"] for r in results["message_results"]) / len(results["message_results"]) if results["message_results"] else 0.0
            }
            ...
            results["conversation_threats"].append({
                "type": "scan_error",
                "severity": "high",
                "description": f"Conversation scanning failed: {str(e)}"
            })
