        ...
        while current_iteration < max_iterations:
            ...
            try:
                ...
                if assistant_message_this_turn:
                    ...
                    )  # type: ignore[dict-item] # Ignore complex type mismatch
                ...
                if (
                    ...
                    messages.append(  # type: ignore[typeddict-item] # Ignore complex type mismatch
                        {"role": "user", "content": turn_tool_results_params}  # type: ignore[typeddict-item] # Ignore complex type mismatch
                    )
                ...
                if is_final_turn:
                    if turn_final_response is not None:
                        ...
                        final_response = (
                            turn_final_response  # Store the final AgentOutputMessage
                        )
                        logger.debug(
                            "Turn processor indicated final turn. Breaking loop."
                        )
                        break  # Exit the loop
                    else:
...
{json.dumps(self.config.config_validation_schema, indent=2)} # Use renamed field

Please correct your previous response to conform to the schema."""
                            correction_message: MessageParam = {
                                "role": "user",
                                "content": correction_message_content,
                            }
                            messages.append(correction_message)
                            conversation_history.append(
                                _serialize_content_blocks(correction_message)
                            )
                            # Continue the loop to let the LLM retry
                        else:
                            # Should not happen if _handle_final_response logic is correct, but log warning
                            logger.error(
                                "Schema validation failed but no schema found in config. Cannot send correction."
                            )
                ...
                try:
                    ...
                    for m in conversation_history:
                        validated_history_for_error.append(
                            AgentOutputMessage.model_validate(
                                _serialize_content_blocks(m)
                            )
                        )
                except Exception as val_err:
                    logger.error(
                        f"Could not validate conversation history for error reporting: {val_err}"
                    )
        ...
        if current_iteration >= max_iterations and final_response is None:
            ...
            if last_assistant_message_dict:
                try:
                    ...
                    final_response = AgentOutputMessage.model_validate(
        ...
        return self._prepare_agent_result(
            conversation_history,
            final_response,
            tool_uses_in_last_turn,
    ...
    def _prepare_agent_result(
        ...
        try:
            ...
            try:
                ...
                # Fallback: Assign empty list as we cannot guarantee the type safety
                validated_conversation_for_error = []
