...
"""
    ...
    def read_changelog(self) -> str:
        ...
        try:
            ...
            raise ChangelogError("Changelog file missing") from err
        except PermissionError as err:
            logger.error("Permission denied reading changelog")
            raise ChangelogError("Permission denied") from err
        except Exception as e:
            logger.error(f"Error reading changelog: {e}")
            raise ChangelogError(f"Failed to read changelog: {e}") from e
    ...
    def write_changelog(self, content: str) -> None:
        ...
        try:
            ...
            raise ChangelogError(f"Failed to write changelog: {e}") from e

    def add_entry(self, section: str, pr_title: str, pr_author: str, pr_num: str, pr_url: str) -> bool:
    ...
    def release_version(self, new_version: str, current_version: str, date: str, repo: str) -> bool:
...
def main():
    ...
    parser.add_argument("--action", choices=["add", "release"], required=True, help="Action to perform")
    ...
    if args.action == "add":
        ...
        success = manager.add_entry(args.section, args.pr_title, args.pr_author, args.pr_num, args.pr_url)
        ...
        success = manager.release_version(args.new_version, args.current_version, args.date, args.repo)
        sys.exit(0 if success else 1)
