...
Write a 2-3 sentence summary highlighting the most significant improvements in this major release. Focus on user-facing features, architectural improvements, or major capabilities added. Be concise and technical but accessible."""
    ...
    return call_claude_api(
        ...
        api_key,
        300,
        "Major release bringing significant improvements and new capabilities to the Aurite Agents Framework.",
    )


def generate_release_body(
    version: str, version_type: str, environment: str, changes: str, changelog_section: str, api_key: str
) -> str:
...
def main():
    ...
    if args.mode == "summary":
        ...
        if not all([args.version, args.version_type, args.environment, args.changes is not None]):
            print(
                "--version, --version-type, --environment, and --changes are required for release mode", file=sys.stderr
            )
            sys.exit(1)
        ...
        release_body = generate_release_body(
            args.version, args.version_type, args.environment, args.changes, args.changelog_section, api_key
        )
        print(release_body)
