...
*   **Registration:** The `HostManager` provides methods (`register_client`, `register_agent`, etc., `register_config_file`) for dynamic registration. These update in-memory configs (managed by `ProjectManager` and `ComponentManager`) and optionally sync changes to the database via `StorageManager`.
*   **Execution:** Entrypoints use `HostManager.execution` (`ExecutionFacade`).
    ...
    *   It manages a cache of LLM clients (`_llm_client_cache`) to reuse instances based on `LLMConfig.llm_id`. If an `AgentConfig` has no `llm_config_id`, a temporary, non-cached client is created using agent-specific LLM parameters or defaults.
