...
SQL_SERVER_PATH = Path(__file__).parent.parent / "src" / "storage" / "sql" / "sql_server.py"
...
async def test_sql_roots_and_resources():
    ...
    try:
        ...
        sql_tools = [t for t in tools if t["name"] in ("connect_database", "execute_query", "list_tables")]
        
        logger.info(f"Found {len(sql_tools)} SQL tools: {[t['name'] for t in sql_tools]}")
        if not sql_tools:
        ...
        explore_prompt = await host.prompts.get_prompt("explore_database_prompt", "sql-server")
        if explore_prompt:
        ...
        if isinstance(connect_result, list) and len(connect_result) > 0:
            ...
            if "Failed to connect" in result_text:
                logger.warning("Database connection failed - this test requires a running PostgreSQL database")
                logger.warning("Skipping remaining database-specific tests, but continuing with other tests")
        ...
        logger.info("Instead, we would access them directly by schema_uri or execute_query tools")
        logger.info(f"For example, sql://schema/{connection_id} would provide the schema")
        ...
        logger.info("Schema was accessed through tools rather than the resource mechanism")
        ...
        try:
            ...
            if not is_allowed:
                logger.warning(f"Schema resource {schema_uri} is not allowed by any registered roots")
        ...
        if api_key and has_real_connection:
            ...
            if 'tool_uses' in response and response['tool_uses']:
                ...
                for i, tool_use in enumerate(response['tool_uses']):
                    logger.info(f"Tool call {i+1}: {tool_use.get('content', '')[:200]}...")
            ...
            logger.warning("Skipping prompt with tools test - ANTHROPIC_API_KEY not set")
        elif not has_real_connection:
            logger.warning("Skipping prompt with tools test - no real database connection")
        
        # Cleanup: Disconnect from database if we have a real connection
        if has_real_connection:
            logger.info("\n=== Cleaning Up ===")
            disconnect_result = await host.tools.execute_tool("disconnect", {"connection_id": connection_id})
            logger.info(f"Disconnect result: {disconnect_result[0].text if disconnect_result else 'Failed'}")
        else:
            logger.info("\n=== Cleaning Up ===")
