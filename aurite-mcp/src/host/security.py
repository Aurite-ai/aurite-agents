...
class SecurityManager:
    ...
    async def register_server_permissions(self, server_id: str, allowed_credential_types: List[str]):
        """Register which credential types a server is allowed to access"""
        self._server_permissions[server_id] = allowed_credential_types
        logger.info(f"Registered permissions for server {server_id}: {allowed_credential_types}")
    ...
    def mask_sensitive_data(self, data: str) -> str:
        ...
        for pattern in SENSITIVE_PATTERNS.values():
            data = re.sub(pattern, lambda m: m.group(0).replace(m.group(1), "*****"), data)
