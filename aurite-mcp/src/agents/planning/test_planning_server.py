...
class PlanningClient:
    ...
    async def test_planning_prompt(self):
        ...
        if hasattr(prompt_result, "messages") and prompt_result.messages:
            prompt_content = (
                prompt_result.messages[0].content if prompt_result.messages else ""
            )
        else:
...
"""
    ...
    async def test_list_plans(self):
        ...
        tool_result = await self.session.call_tool("list_plans", {})
    ...
    async def test_filtered_plans(self, tag):
        ...
        tool_result = await self.session.call_tool("list_plans", {"tag": tag})
    ...
    async def test_plan_resource(self, plan_name):
        ...
        resource_result = await self.session.read_resource(
            f"planning://plan/{plan_name}"
        )
    ...
    async def test_analysis_prompt(self, plan_name):
        ...
        prompt_result = await self.session.get_prompt(
            "plan_analysis_prompt", {"plan_name": plan_name}
        )
    ...
    async def test_with_llm(self, plan_name):
        ...
        response = await self.process_query(
            "Create a detailed project plan for building a mobile weather app. "
            + "The plan should be comprehensive and follow a structured format."
        )
