"""
Utility functions and interfaces for agent implementations.
...
4. Abstract interfaces for tools and tool providers
5. Retry decorator for async functions
"""
...
import time
import functools
from typing import Dict, List, Any, Set, Optional, Callable, TypeVar, Awaitable, Protocol, runtime_checkable
...
@runtime_checkable
class Tool(Protocol):
    """Protocol defining the interface for a tool that can be used by agents"""
...
class ToolProvider(ABC):
    """Abstract base class for providing tools to agents"""
...
def validate_required_fields(data: Dict[str, Any], required_fields: Set[str], context_name: str = "Context") -> bool:
    """
...
def validate_provided_outputs(outputs: Dict[str, Any], provided_outputs: Set[str], context_name: str = "Step") -> bool:
    """
...
def summarize_execution_results(
    ...
    if status_enum is None:
        ...
        steps_failed = len([
            r for r in step_results.values() 
            if getattr(r, 'status', None) == _failed
        ])
        steps_skipped = len([
            r for r in step_results.values() 
            if getattr(r, 'status', None) == _skipped
        ])
        ...
        async def wrapper(*args, **kwargs):
            ...
            while attempts <= max_retries:
                ...
                try:
                    ...
                    logger.warning(f"Function '{func.__name__}' failed on attempt {attempts}: {e}")
                    last_exception = e
                    
                    # Check if we should retry for this exception
                    if should_retry and not should_retry(e):
                        logger.info(f"Not retrying '{func.__name__}': exception {type(e).__name__} not eligible for retry")
                        break
