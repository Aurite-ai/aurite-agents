...
class BasicEvaluationWorkflow(BaseWorkflow):
    """A simplified version of the planning workflow for testing"""

    def __init__(self, tool_manager, host: MCPHost, state: str):
        super().__init__(tool_manager, name="basic_planning_workflow", host=host)
        self.description = "Basic evaluation workflow for testing"
        self.state = state

    async def perform_simple_evaluation(self, state, plan_name):
        ...
        result = await self.host.execute_prompt_with_tools(
            prompt_name="evaluation_prompt",
            prompt_arguments={
                "state": state
            },
            client_id="evaluation",
            user_message="Evaluate the agent using the evaluate_agent tool.",
            tool_names=["evaluate_agent"],
            model="claude-3-sonnet-20240229",
        )

        # Return the result
        return {"success": True, "plan_name": plan_name, "result": result}
...
async def main():
    ...
    if not os.environ.get("ANTHROPIC_API_KEY"):
        ...
        logger.warning(
            "Set it before running this test, or the prompt execution will fail."
        )
        return
    ...
    host_config = HostConfig(
        clients=[
            ClientConfig(
                client_id="evalutaion",  # This must match the client_id in the workflow
    ...
    try:
        ...
        workflow = BasicEvaluationWorkflow(tool_manager=host.tools, host=host)

        # Create a simple plan
        logger.info("Creating a simple plan...")
        result = await workflow.create_simple_plan(
            task="Test the planning workflow", plan_name="simple_test_plan"
        )
