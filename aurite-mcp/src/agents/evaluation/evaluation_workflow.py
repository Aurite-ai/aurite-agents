...
class EvaluateAgentStep(WorkflowStep):
    ...
    async def execute(self, context: AgentContext) -> Dict[str, Any]:
        ...
        logger.info(f"Evaluating agent output with {len(rubric.get('criteria', {}))} criteria")
        ...
        try:
            ...
            logger.info(f"Evaluation complete with score: {evaluation_result.get('score', 'unknown')}")
...
class AggregateEvaluationsStep(WorkflowStep):
    ...
    async def execute(self, context: AgentContext) -> Dict[str, Any]:
        ...
        # Otherwise, perform aggregation in-memory using the provided results
            ...
            logger.info(f"Aggregating {len(evaluation_results)} evaluation results in-memory")
        ...
        try:
            ...
            logger.info(f"Aggregation complete for {aggregated_result.get('num_runs', 0)} evaluations")
    ...
    def _compute_aggregated_stats(self, evaluation_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Compute aggregated statistics from a list of evaluation results."""
        # Extract overall scores
        scores = [result.get("score", 0) for result in evaluation_results if "score" in result]
        if not scores:
        ...
        common_strengths = [s for s, count in strength_counts.items() if count >= threshold]
