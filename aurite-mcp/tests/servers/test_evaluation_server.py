...
async def test_evaluation_server_tools(test_mcp_host):
    ...
    for tool_name in expected_tools:
        ...
        assert (
            tool_exists
        ), f"Expected tool '{tool_name}' not found in evaluation server"

    # Check for tool schema and details
    if any(t["name"] == "evaluate_agent" for t in tools):
        tool_schema = next((t for t in tools if t["name"] == "evaluate_agent"), {}).get(
            "inputSchema", {}
        )
...
async def test_evaluation_server_prompts(test_mcp_host):
    ...
    for prompt_name in expected_prompts:
        ...
        assert (
            prompt_exists
        ), f"Expected prompt '{prompt_name}' not found in evaluation server"

    # Test prompt content
    if prompts:
        prompt_details = await tester.host.prompts.get_prompt(
            "evaluation_prompt", tester.client_id
        )
...
async def test_evaluation_server_resources(test_mcp_host):
    ...
    client_tools = [
        t for t in tester.tools if t["name"] in ["evaluate_agent", "score_agent"]
    ]
