...
async def test_planning_server_tools():
    ...
    try:
        ...
        for tool_name in expected_tools:
            ...
            assert tool_exists, (
                f"Expected tool '{tool_name}' not found in planning server"
            )
    ...
    try:
        ...
        for prompt_name in expected_prompts:
            ...
            assert prompt_exists, (
                f"Expected prompt '{prompt_name}' not found in planning server"
            )

        # Test prompt content (optional)
        if prompts:
            prompt_details = await tester.host.prompts.get_prompt(
                "create_plan_prompt", tester.client_id
            )
            assert prompt_details.description, (
                "Planning prompt should have a description"
            )
...
async def test_planning_server_resources():
    ...
    try:
        ...
        planning_client = next(
            (c for c in client_configs if c.client_id == "planning"), None
        )
        if planning_client:
            logger.info(
                f"Planning client found with roots: {[r.uri for r in planning_client.roots]}"
            )
            has_planning_root = any(
                r.uri.startswith("planning://") for r in planning_client.roots
            )
