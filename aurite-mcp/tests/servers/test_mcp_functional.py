...
class MCPServerTester:
    ...
    def prepare_host_config(self) -> HostConfigModel:
        ...
        if agent_config:
            ...
            logger.info(
                f"No JSON configuration found for {self.client_id}, using defaults"
            )
            client_config = ClientConfig(
                client_id=self.client_id,
                server_path=self.server_path,
                roots=[
                    ...
                    RootConfig(
                        name="test", uri="test:///", capabilities=["read", "write"]
                    ),
                ],
                capabilities=["tools", "prompts", "resources", "roots"],
                timeout=30.0,
    ...
    async def initialize_host(self, config: Optional[HostConfigModel] = None) -> None:
        ...
        if self.using_existing_host:
            logger.info(
                f"Using existing host with {len(self.host.tools.list_tools())} tools"
            )
            return
    ...
    async def discover_components(self) -> Dict[str, List]:
        ...
        try:
            ...
            logger.info(
                f"Found {len(self.prompts)} prompts: {[p.name for p in self.prompts]}"
            )
    ...
    async def test_tools(self) -> Dict[str, Any]:
        ...
        for tool in self.tools:
            ...
            try:
                ...
                if properties:
                    prop_list = [
                        f"{p} ({'required' if p in required_params else 'optional'})"
                        for p in properties.keys()
                    ]
                    logger.info(
                        f"Tool '{tool_name}' parameters: {', '.join(prop_list)}"
                    )
                else:
                    logger.info(
                        f"Tool '{tool_name}' has no defined parameters in schema"
                    )
    ...
    async def test_prompts(self) -> Dict[str, bool]:
        ...
        for prompt in self.prompts:
            ...
            try:
                ...
                prompt_details = await self.host.prompts.get_prompt(
                    prompt_name, self.client_id
                )
                logger.info(
                    f"Prompt '{prompt_name}' description: {prompt_details.description}"
                )
...
async def run_mcp_server_test(
    server_path: str, shared_host: Optional[MCPHost] = None
    ...
    try:
        ...
        if results["tools"]:
            tool_success = all(
                result.get("success", False) for result in results["tools"].values()
            )
        ...
        # Report on resources
        logger.info(
            f"Resources {'found' if results['resources']['found'] else 'not found'}"
        )
...
async def test_mcp_server_functional(server_path: str, test_mcp_host) -> None:
    ...
    has_components = (
        bool(results["tools"])
        or bool(results["prompts"])
        or results["resources"]["found"]
    )
    ...
    if results["tools"]:
        for tool_name, tool_result in results["tools"].items():
            assert tool_result.get(
                "success", False
            ), f"Tool '{tool_name}' is not accessible"
