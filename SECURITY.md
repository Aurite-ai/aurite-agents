...
- **Security Considerations:**
  ...
  - **Logging:** The Aurite Agents framework (specifically `ClientManager` and `MCPHost`) **does not log the values of these environment variables** (including resolved secrets) during the startup process. It logs that secrets _are_ being injected and how many, but not their content.
...
The `HostManager` (`src/aurite.py`) provides methods for dynamically registering new components (Clients, Agents, LLMs, Linear Workflows, and Custom Workflows) at runtime, typically via API endpoints.
