...
def base_llm_config() -> LLMConfig:
    ...
    return LLMConfig(
        name="test_llm", provider="anthropic", model="claude-3-haiku-20240307"
    )
...
async def test_agent_run_conversation_with_tool_use(
    ...
    llm_response_turn_1 = ChatCompletionMessage(
        role="assistant", content=None, tool_calls=[tool_call]
    )

    # Turn 2: LLM provides the final answer based on the tool result
    final_text_response = "The user's name is Alice."
    llm_response_turn_2 = ChatCompletionMessage(
        role="assistant", content=final_text_response, tool_calls=None
    )
    ...
    mocker.patch(
        "aurite.components.agents.agent.LiteLLMClient", return_value=mock_llm_instance
    )

    # Configure the mock host to return a result for the tool call
    mock_host.get_formatted_tools.return_value = [
        {"name": "get_user_info", "input_schema": {}}
    ]
    ...
    mock_host.call_tool.assert_awaited_once_with(
        name="get_user_info", args={"user_id": "123"}
    )
