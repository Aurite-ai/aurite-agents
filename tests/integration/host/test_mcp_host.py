...
async def test_mcp_host_initializes_and_connects_via_session_group(
    host_config, mock_client_session_group, mocker
):
    """
    ...
    host_config.mcp_servers = [client_config1, client_config2]
...
async def test_get_server_params_handles_env_vars_and_docker(
    ...
    host_config.mcp_servers = [client_config]
    ...
    # 2. Act
    async with MCPHost(config=host_config):
        # 3. Assert
