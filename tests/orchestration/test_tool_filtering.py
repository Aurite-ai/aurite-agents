...
from src.host.models import AgentConfig  # Added import
...
def tool_manager():
    ...
    tm._tools = {"tool1": tool1, "tool2": tool2, "tool3": tool3}
...
def test_format_tools_for_llm_all(tool_manager):
    mock_filtering_manager = MagicMock(spec=FilteringManager)
    # Mock the methods used within format_tools_for_llm if necessary,
    # but format_tools_for_llm should handle None agent_config gracefully.
    # Let's assume the filtering manager's methods return expected values for None config.
    mock_filtering_manager.filter_clients_for_request.return_value = (
        None  # No client filtering
    )
    mock_filtering_manager.filter_component_list.side_effect = (
        lambda l, c: l
    )  # No component filtering

    tools = tool_manager.format_tools_for_llm(
        filtering_manager=mock_filtering_manager, agent_config=None
    )
    assert len(tools) == 3
    # Check the actual tool names returned, as order isn't guaranteed
    returned_names = {tool["name"] for tool in tools}
    assert returned_names == {"tool1", "tool2", "tool3"}


def test_format_tools_for_llm_filtered(tool_manager):
    mock_filtering_manager = MagicMock(spec=FilteringManager)
    agent_config = AgentConfig(client_ids=["client2", "client3"])

    # Mock filtering manager behavior for this agent config
    mock_filtering_manager.filter_clients_for_request.return_value = [
        "client2",
        "client3",
    ]
    mock_filtering_manager.filter_component_list.side_effect = (
        lambda l, c: l
    )  # No component filtering for this test

    tools = tool_manager.format_tools_for_llm(
        filtering_manager=mock_filtering_manager, agent_config=agent_config
    )
    assert len(tools) == 2
    returned_names = {tool["name"] for tool in tools}
    assert returned_names == {"tool2", "tool3"}


def test_format_tools_for_llm_filtered_name(tool_manager):
    mock_filtering_manager = MagicMock(spec=FilteringManager)
    # Agent config allows client2 and client3
    agent_config = AgentConfig(client_ids=["client2", "client3"])
    ...
    # Request specific tools, filtering happens *after* client filtering
    tools = tool_manager.format_tools_for_llm(
        filtering_manager=mock_filtering_manager,
        agent_config=agent_config,
        tool_names=["tool2", "tool1"],  # Only request tool1 and tool2
    )
    assert len(tools) == 1
    returned_names = {tool["name"] for tool in tools}
    assert returned_names == {"tool2"}  # Only tool2 matches client_ids AND tool_names
