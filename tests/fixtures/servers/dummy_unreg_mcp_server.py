...
async def _dummy_list_tools_handler() -> list[types.Tool]: # Return list[types.Tool]
    tool_instance = types.Tool(name=TOOL_NAME, description="A test tool for unregistration.", inputSchema={})
    return [tool_instance]

async def _dummy_call_tool_handler(name: str, arguments: dict) -> types.CallToolResult:
    return types.CallToolResult(content=[types.TextContent(type="text", text="dummy result from " + name)])
