...
async def test_host_initializes_http_stream_client_successfully(
    ...
    assert mcp_host.is_client_registered(http_client_id), (
    ...
    assert session is not None, (
        f"Session for HTTP Stream client '{http_client_id}' should be active."
    )
    ...
    tool_names_from_router = await mcp_host._message_router.get_tools_for_client(
        http_client_id
    )
    ...
    assert "uppercase_text" in tool_names_from_router, (
        f"Tool 'uppercase_text' not discovered for client '{http_client_id}'"
    )
    assert "add_numbers" in tool_names_from_router, (
        f"Tool 'add_numbers' not discovered for client '{http_client_id}'"
    )
    assert len(tool_names_from_router) == 2, (
        f"Expected 2 tools for client '{http_client_id}', found {len(tool_names_from_router)}"
    )
