...
from src.host.foundation import SecurityManager, RootManager, MessageRouter
from src.host.resources import PromptManager, ResourceManager, ToolManager
...
def mock_client_session() -> MagicMock:
    ...
    mock.initialize = AsyncMock(return_value=MagicMock(capabilities=[])) # Mock initialize response
...
async def test_initialize_client_connection_error(
    ...
    mock_stdio_client.return_value.__aenter__.side_effect = ConnectionRefusedError("Failed to connect")
    ...
    with pytest.raises(ConnectionRefusedError, match="Failed to connect"):
        await host.initialize() # This will call initialize on real managers, then fail on stdio_client
...
async def test_initialize_manager_init_error(
    ...
    with patch.object(host._tool_manager, 'initialize', side_effect=RuntimeError("Tool manager failed")):
...
async def test_shutdown(
    ...
    with patch.object(host._tool_manager, 'shutdown', wraps=host._tool_manager.shutdown) as spy_tool_shutdown, \
         patch.object(host._resource_manager, 'shutdown', wraps=host._resource_manager.shutdown) as spy_resource_shutdown, \
         patch.object(host._prompt_manager, 'shutdown', wraps=host._prompt_manager.shutdown) as spy_prompt_shutdown, \
         patch.object(host._message_router, 'shutdown', wraps=host._message_router.shutdown) as spy_router_shutdown, \
         patch.object(host._root_manager, 'shutdown', wraps=host._root_manager.shutdown) as spy_root_shutdown, \
         patch.object(host._security_manager, 'shutdown', wraps=host._security_manager.shutdown) as spy_security_shutdown:
