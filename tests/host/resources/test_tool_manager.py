...
async def test_unregister_client_tools(
    tool_manager: ToolManager,
    mock_message_router: MagicMock,
    mock_filtering_manager: MagicMock,
    ...
    # get_tools_for_client(client_id_b) -> {"common_tool"}
    # get_clients_for_tool("common_tool") -> [client_id_a, client_id_b]
    # get_clients_for_tool("tool_a_only") -> [client_id_a]
    async def mock_get_tools_for_client(client_id):
        if client_id == client_id_a:
            return {"common_tool", "tool_a_only"}
    ...
    async def mock_get_clients_for_tool(tool_name):
        if tool_name == "common_tool":
            return [client_id_a, client_id_b]
    ...
    assert "tool_a_only" in tool_manager._tools
    ...
    assert "common_tool" in tool_manager._tools
    assert "common_tool" in tool_manager._tool_metadata
    # tool_a_only should be removed as client A was the only provider
    assert "tool_a_only" not in tool_manager._tools
    assert "tool_a_only" not in tool_manager._tool_metadata
